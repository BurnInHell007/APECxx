...
namespace wav_reader
{
    ...
    /**
     * Template specialization : explicitly mentions to use
     * this function instead when float is used
     * and floats are represented in range [-1.0, 1.0]
     */
    template <>
    ...
    // 2^(16)/2^(8) = 2^(8) so divide it with 256
    template <>
